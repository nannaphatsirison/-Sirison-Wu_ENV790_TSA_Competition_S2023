...
After aggregated data and analyzed original seasonality of time series, we should first forecast daily load before we fit models. We created two time series: one is trained data started from January 1st,2015 and end on December 31st,2019; and the other is tested data from January 1st,2020 to December 31st,2020. The trained time series is created to fit models, and the tested time series is used for checking the accuracy of our models as a year of data out-of-sample analysis.
...
autoplot(ts) +
  ...
  ylab("Hourly Load")
```
We tried different K numbers, such as 4, 6, 12, 24, 36, and 60. We found that although 48 performs the best accuracy compared with the original data, the observed data still have a greater magnitude than the forecasted data.
